...
p4 = ggplot(dtf_com, aes(x =log_gdp , y =log_pgr)) + 
  xlab("logGDP") + 
  ylab("LogPG")+
  geom_point()+
  theme_classic()+
...
p2 = ggplot(dtf_com, aes(x = log_gdp, y = log_b)) + 
  ...
  stat_regline_equation(label.y = 7, aes(label= ..rr.label..))
p2

library(cowplot)

plot_grid(p2, p4, ncol=1, 
          rel_heights = c(1, 1, -0.05, 1), label_size = 10)
...
head(urb_dtf)
dput(urb_dtf)         # only to upload que in stake overflow 
library(writexl)
#write_xlsx(as.data.frame(urb_dtf), path= "ur data.xlsx")
view(shp_dt)
urb_dtf
df_mp= data.frame( urb_dt$UC_NM_MN, urb_dt$GCPNT_LON, urb_dt$GCPNT_LAT, urb_dt$P15)
head(df_mp)
#view(df_mp)
rename(dt_f_mp, Name_3= urb_dt.UC_NM_MN)
